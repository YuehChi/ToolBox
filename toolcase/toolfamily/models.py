...
model field 語法筆記
    ...
    - default 表示預設的填入值
    - blank=True 表示允許創建時給定空值。請搭配 null=True，否則會很容易出錯。
...
class CasePhoto(models.Model):
    casephoto_id = models.AutoField(primary_key=True)
...
class CaseWillingness(models.Model):
    # ID與關係人
    casewillingness_id = models.AutoField(primary_key=True)
    apply_case = models.ForeignKey(
        Case,
        related_name='applyed_by',
        verbose_name='委託單',
        on_delete=models.CASCADE)
    willing_user = models.ForeignKey(
        UserDetail,
        related_name='applys',
        verbose_name='應徵的工具人',
        on_delete=models.CASCADE)
...
class CommissionRecord(models.Model):
    # ID與關係人、關係案件
    commissionrecord_id = models.AutoField(primary_key=True)
    case = models.ForeignKey(
        Case,
        related_name='commission_record',
        verbose_name='委託單',
        on_delete=models.CASCADE)
    commissioned_user = models.ForeignKey(
        UserDetail,
        related_name='commission_record',
        verbose_name='接案的工具人',
        on_delete=models.CASCADE)
...
class ReportType(models.Model):
    ...
    def __str__(self):  # 拿去 print() 時要怎麼顯示
        return f'({self.report_type_id}) {self.report_type_name}'
...
class Report(models.Model):
    ...
    reporter = models.ForeignKey(
        ...
        verbose_name='檢舉人',
        on_delete=models.CASCADE)
    reported_case = models.ForeignKey(
        Case,
        related_name='beReported',
        null=True, blank=True,
